...
@variables Qt [description = "Routed outflow", unit = "mm/d"]
@variables t
...
model_parameters = [TT, TTI, TTM, CFR, CFMAX, WHC, CFLUX, FC, LP, BETA, K0, ALPHA, PPERC, K1, MAXBAS]
...
uh_1 = @unithydro begin
    uh_func = begin
        MAXBAS => 1 / (0.5 * MAXBAS^2) * (0.5 * min(t, MAXBAS)^2 - 0.5 * (t - 1)^2)
    end
    uh_vars = q => Qt
end
