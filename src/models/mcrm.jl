...
@variables uib_before [description = "delayed flow", unit = "mm/d"]
...
@variables Dexcess [description = "excess storage", unit = "mm"]
model_variables = [S, D, Sg, Sic, Soc, P, Ec, qf, Ep, qn, Er, qd, qp, qr, qb, Qt, uib, qic, uob, qoc, Dexcess]
# Model parameters
...
@parameters tau [description = "Routing delay  ", bounds = (1, 10), unit = "d"]
...
model_parameters = [Smax, cmax, ct, c1, ce, dsurp, kd, gamd, qpmax, kg, tau, sbf, kcr, gamcr, kor, gamor]
...
bucket2 = @hydrobucket :bucket2 begin
    fluxes = begin
        ...
        @hydroflux Dexcess ~ max(0.0, D - dsurp)
        @hydroflux qd ~ min(Dexcess, kd * Dexcess^gamd)
        @hydroflux qp ~ min(D, clamp(D / dsurp, 0, 1) * qpmax)
    end
    dfluxes = begin
...
bucket3 = @hydrobucket :bucket3 begin
    fluxes = begin
        @hydroflux qb ~ min(max(0.0, Sg), kg * max(0.0, Sg)^1.5)
        @hydroflux uib_before ~ qr + qd + qb
    end
    dfluxes = begin
        @stateflux Sg ~ qp - qb
    end
...
uh_1 = HydroModels.UnitHydrograph(
    [uib_before], [uib], [tau],
    (t, p) -> begin
        lag = p.params.tau
        if t == ceil(lag)
            return (lag % (t - 1)) / lag + (t - 1) / lag
        else
            return t / lag
        end
    end,
    (p) -> ceil(p.params.tau), name=:uh_1
)

bucket4 = @hydrobucket :bucket4 begin
    fluxes = begin
        @hydroflux uob ~ (1 - step_func(Sic - sbf)) * uib
        @hydroflux qic ~ min(3 / 4 * max(0.0, Sic), kcr * max(0.0, Sic)^gamcr)
        @hydroflux qoc ~ min(3 / 4 * max(0.0, Soc), kor * max(0.0, Soc)^gamor)
    end
    dfluxes = begin
        @stateflux Sic ~ qb - uob - qic
