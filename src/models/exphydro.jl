...
@variables soilwater Ep evap baseflow surfaceflow Qt rainfall
model_variables = [T, Ep, P, snowfall, rainfall, snowpack, melt, soilwater, Ep, evap, baseflow, surfaceflow, Qt, rainfall]
...
model_parameters = [f, Smax, Qmax, Df, Tmax, Tmin]
...
bucket_2 = @hydrobucket :soil begin
    fluxes = begin
        ...
        @hydroflux Qt ~ baseflow + surfaceflow
    end
    dfluxes = begin
        @stateflux soilwater ~ (rainfall + melt) - (evap + Qt)
    end
end
