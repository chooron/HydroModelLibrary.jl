module gsmsocont
using ..HydroModels
using ..HydroModels: step_func, smoothlogistic_func
...
model_variables = [His, Pices, Mis, T, P, Ep, Pice, Mis, Ts, Sis, Qis, Pirs, Picer, Pnis, Mnis, Snis, Pinf, ET, Qsl, Pinf, Peff, Peq, Mnis, Sniq, Qt]
...
@parameters aice [description = "Degree-day factor for ice melt", bounds = (0, 1), unit = "mm/oC/d"]
...
model_parameters = [fice, t0, asnow, tm, ks, aice, ki, a, x, y, ksl, beta]

ice_split_flux = @hydroflux :ice_split_flux begin
    Pice ~ fice * P
    Pnonice ~ (1 - fice) * P
end

pice_split_flux = @hydroflux :pice_split_flux begin
    Pices ~ step_func(t0 - T) * Pice
    Picer ~ step_func(T - t0) * Pice
end

pnonice_split_flux = @hydroflux :pnonice_split_flux begin
    Pnir ~ step_func(T - t0) * Pnonice
    Pnis ~ step_func(t0 - T) * Pnonice
end

bucket1 = @hydrobucket :bucket1 begin
    fluxes = begin
        @hydroflux Mis ~ min(His, asnow * max(0.0, T - tm))
        @hydroflux Mii ~ smoothlogistic_func(His, 0.1) * aice * max(0.0, T - tm)
...
bucket2 = @hydrobucket :bucket2 begin
    fluxes = begin
        ...
        @hydroflux Qqu ~ min(Sniq, beta * max(0.0, Sniq)^(5 / 3))
    end
    dfluxes = begin
