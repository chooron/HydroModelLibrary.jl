...
@variables Excess [description = "excess soil storage", unit = "-"]
...
model_variables = [SI, P, Ep, EI, PEX, PEXMP, SMP, Excess, PEXS1, FMP, QMP, QEXMP, PQEXS1, SS1, FS1, QS1, ETAS1, FF, Q0, Q0_routed, QMPS1, SS2, PC, QH, QS2, Qt, Q0R]
# Model parameters
...
model_parameters = [SIMAX, A, FF, SMPMAX, CQMP, XQMP, SS1MAX, FCCS1, CFS1, XFS1, CQS1, XQS1, SS2MAX, CQS2, XQS2, D0]
...
bucket3 = @hydrobucket :bucket3 begin
    fluxes = begin
        ...
        @hydroflux Excess ~ max(0.0, SS1 - FCCS1 * SS1MAX)
        @hydroflux QS1 ~ clamp(CQS1 * max(0.0, Excess)^XQS1, 0.0, Excess)
...
uh_1 = HydroModels.@unithydro :uh_1 begin
    uh_vars = Q0 => Q0_routed
    uh_func = begin
        D0 => 1 / (0.5 * D0^2) * (0.5 * min(t, D0)^2 - 0.5 * (t - 1)^2)
    end
end
...
bucket4 = @hydrobucket :bucket4 begin
    fluxes = begin
        ...
        @hydroflux QS2 ~ min(SS2 - SS2MAX, CQS2 * max(0.0, SS2 - SS2MAX)^XQS2)
        @hydroflux QH ~ min(QMPS1 - PC, 0.0)
