...
model_variables = [Pin, Ea, qex1, Pn, P, Ep, Sdef, Et, qex2, Suz, Pby, quz, Qt, Ssz, Srz]
# Model parameters
...
@parameters k2 [description = " Runoff coefficient", bounds = (-3, 0), unit = "1/d"]
model_parameters = [ca, phi, rc, gam, k1, fa, k2]
...
bucket4 = @hydrobucket :bucket4 begin
    fluxes = begin
        @hydroflux Qt ~ min(10^k2 * (Ssz^2), max(0.0, Ssz))
    end
    dfluxes = begin
