...
model_variables = [S, P, Ep, Eveg, Ebs, Qss, Qse, Qbf, Qt]
# Model parameters
...
model_parameters = [Smax, Sfc, M, a, b, tc_bf]
# Soil water component
soil_bucket = @hydrobucket :soil begin
    fluxes = begin
        @hydroflux Eveg ~ min(S / (Sfc * Smax), 1.0) * M * Ep
        @hydroflux Ebs ~ S / Smax * (1 - M) * Ep
        @hydroflux Qse ~ P * step_func(S - Smax)
        @hydroflux Qss ~ min((a * max(0.0, S - Sfc * Smax))^b, max(0.0, S - Sfc * Smax))
