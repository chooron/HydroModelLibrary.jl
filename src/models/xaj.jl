...
@variables Ep [description = "Potential evapotranspiration"]
...
@variables Qt [description = "Channel network outflow intensity"]
...
model_variables = [P, pn, iu, il, fw, Ep, en, eu, el, ed, et, r, rs, ri, rg, rt, qi, qg, qt, q1, q2, q3, Qt, wu, wl, wd, s0, oi, og, F1, F2, F3]
# Model parameters
...
soil_bucket = @hydrobucket :soil begin
    fluxes = begin
        ...
        @hydroflux fw ~ (1 - Aimp) * (1 - clamp(1 - (wu + wl + wd) / (Wum + Wlm + Wdm), 0, 1)^(b / (1 + b)))
...
free_water_bucket = @hydrobucket :zone begin
    fluxes = begin
        @hydroflux rs ~ pn * fw * (1 - clamp(1 - s0 / Smax, 0, 1)^(ex / (1 + ex)))
        @hydroflux begin
            ri ~ fw * s0 * (-Ki * log(max(1e-6, 1 - Ki - Kg)) / (Ki + Kg))
            rg ~ fw * s0 * (-Kg * log(max(1e-6, 1 - Ki - Kg)) / (Ki + Kg))
        end
