...
@variables Wprop [description = "proportion of tension water storage", unit = "-"]
...
model_variables = [W, Pi, E, R, P, Wprop, Ep, LM, S, RS, RI, RG, Rprop, Smax, SI, QI, SG, QG, RB, QS, Qt]
# Model parameters
...
model_parameters = [Aim, a, b, Wmax, LM, c, Smax, Ex, kI, kG, cI, cG]
# Soil water component
bucket_1 = @hydrobucket :bucket_1 begin
    fluxes = begin
        @hydroflux Pi ~ (1 - Aim) * P
        @hydroflux Wprop ~ clamp(W / Wmax, 0.0, 1.0)
        @hydroflux R ~ clamp(step_func(Wprop - (0.5 - a)) *
                             (Pi * (0.5 - a)^(1 - b) * Wprop^b) +
                             step_func((0.5 - a) - Wprop) *
                             (Pi * (1 - (0.5 + a)^(1 - b) * (1 - Wprop)^b)), 0.0, Pi)
