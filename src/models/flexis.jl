...
@variables t
model_variables = [UR, Ru, Eur, Rp, Cr, P, Ep, Ps, Rs, Rf, FR, SR, Rfl, Rsl, Qf, Qs, Qt]
# Model parameters
...
@parameters Imax [description = "Maximum interception storage", bounds = (0, 5), unit = "mm"]
@parameters TT [description = "Threshold temperature for snowfall/snowmelt", bounds = (-3, 5), unit = "oC"]
@parameters ddf [description = "Degree-day factor for snowmelt", bounds = (0, 20), unit = "mm/d/oC"]
model_parameters = [URmax, beta, D, Percmax, Lp, Nlagf, Nlags, Kf, Ks]
bucket1 = @hydrobucket :bucket1 begin
    fluxes = begin
        @hydroflux Ps ~ P * step_func(TT - T)
        @hydroflux M ~ ddf * max(0.0, T - TT)
    end
    dfluxes = begin
        @stateflux S ~ Ps - M
    end
...
bucket2 = @hydrobucket :bucket2 begin
    fluxes = begin
        @hydroflux Pi ~ step_func(T - TT) * P
        @hydroflux Ei ~ Ep * step_func(I)
        @hydroflux Peff ~ Pi * step_func(I - Imax)
    end
    dfluxes = begin
        @stateflux I ~ Pi + M - Ei - Peff
    end
end

flux1 = @hydroflux :split_flux begin
    Rs ~ (P - Ru) * D + Ps
    Rf ~ (P - Ru) * (1 - D)
end
